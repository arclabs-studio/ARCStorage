...
struct ImageCompressorTests {
    @Test("Compress returns data for valid image") func compress_returnsDataForValidImage() {
        // Given — minimal 1x1 JPEG
        let jpeg = makeMinimalJPEG()

        // When
        let result = ImageCompressor.compress(jpeg)

        // Then — output is non-empty valid data
        #expect(!result.isEmpty)
    }

    @Test("Compress returns original data for invalid input") func compress_returnsOriginalForInvalidInput() {
        // Given — garbage bytes
        let garbage = Data([0x00, 0x01, 0x02, 0x03])

        // When
        let result = ImageCompressor.compress(garbage)

        // Then — returns input unchanged (graceful fallback)
        #expect(result == garbage)
    }

    @Test("Compress preserves small images without upsizing") func compress_preservesSmallImages() {
        // Given — 1x1 JPEG is well below 1200px max dimension
        let jpeg = makeMinimalJPEG()

        // When
        let result = ImageCompressor.compress(jpeg, maxDimension: 1200, quality: 0.8)

        // Then — output exists (re-encoded but not upsized)
        #expect(!result.isEmpty)
    }
