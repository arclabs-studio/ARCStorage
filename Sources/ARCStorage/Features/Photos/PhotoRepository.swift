...
public protocol PhotoRepository: AnyObject {
    /// Generates a thumbnail synchronously on `@MainActor`, stores the photo, and returns the persisted `ARCPhoto`.
    ///
    /// Thumbnail generation (200×200 JPEG) takes < 10 ms and runs synchronously to keep
    /// the method non-async. This prevents the Swift runtime from hopping off `@MainActor`
    /// at an `await` point, which previously caused EXC_BREAKPOINT in `modelContext.insert()`.
