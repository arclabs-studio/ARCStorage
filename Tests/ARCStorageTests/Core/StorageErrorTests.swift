...
@Suite("StorageError Tests") struct StorageErrorTests {
    ...
    @Test("saveFailed error description contains save") func saveFailedError_containsSave() {
        let error = StorageError.saveFailed(underlying: NSError(domain: "test", code: 1))
        #expect(error.errorDescription?.contains("save") == true)
    }

    @Test("fetchFailed error description contains fetch") func fetchFailedError_containsFetch() {
    ...
    @Test("deleteFailed error description contains delete") func deleteFailedError_containsDelete() {
    ...
    @Test("invalidData error description contains invalid") func invalidDataError_containsInvalid() {
