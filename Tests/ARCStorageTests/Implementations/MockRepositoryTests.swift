...
@Suite("MockRepository Tests") struct MockRepositoryTests {
    ...
    @Test("Fetch all tracking records call") func fetchAllTracking_recordsCall() async throws {
        let repository = MockRepository<TestModel>()
    ...
    @Test("Fetch tracking records call and ID") func fetchTracking_recordsCallAndID() async throws {
        let repository = MockRepository<TestModel>()
    ...
    @Test("Delete tracking records call and ID") func deleteTracking_recordsCallAndID() async throws {
        let repository = MockRepository<TestModel>()
        await repository.setMockEntities([TestModel.fixture1])
    ...
    @Test("Error simulation throws configured error") func errorSimulation_throwsConfiguredError() async throws {
        let repository = MockRepository<TestModel>()
    ...
    @Test("Reset clears all state") func reset_clearsAllState() async {
        let repository = MockRepository<TestModel>()
        await repository.setMockEntities(TestModel.allFixtures)
        try? await repository.save(TestModel.fixture1)
